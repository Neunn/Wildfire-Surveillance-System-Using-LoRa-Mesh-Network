...
//     if (Mesh_Manager.recvfromAckTimeout(BUF, &BUF_LEN, 10000, &FROM))
//     {
//         Screen_display.clear();
//         Screen_display.drawString(0, 0, "Node : " + String(SelfAddress));
...
//             Screen_display.clear();
//             Screen_display.drawString(0, 0, "Node : " + String(SelfAddress));
//             Screen_display.drawString(0, 10, "Send reply back to : " + String(FROM));
//             Screen_display.display();
//             Serial.println();
//             Serial.println("Send reply back to : " + String(FROM));
//             Serial.println();
//             delay(500);
//         }
//         else
...
void SendDataToGateway(float TEMPERATURE)
{
    ...
    if (Mesh_Manager.sendtoWait((uint8_t *)MESSAGE_CHAR, sizeof(MESSAGE_CHAR), GatewayAddress))
    {
        ...
        uint8_t FROM;
        if (Mesh_Manager.recvfromAck(BUFFER, &BUFFER_LEN, &FROM))
        {
            Screen_display.clear();
...
void RecieveAndRoute()
{
    ...
    /* รับข้อความ หรือ route ข้อความไปยัง Node ถัดไป */
    if (Mesh_Manager.recvfromAckTimeout(BUFFER, &BUFFER_LEN, 12000,&FROM_HOP))
    {
        Screen_display.clear();
        Screen_display.drawString(0, 0, "Node : " + String(SelfAddress));
        ...
        //     Screen_display.clear();
        //     Screen_display.drawString(0, 0, "Node : " + String(SelfAddress));
        //     Screen_display.drawString(0, 10, "Send reply back to : " + String(FROM_HOP, HEX));
        //     Screen_display.display();
        //     Serial.println();
        //     Serial.println("Send reply back to : " + String(FROM_HOP, HEX));
        //     Serial.println();
        // }
        // else
        // {
