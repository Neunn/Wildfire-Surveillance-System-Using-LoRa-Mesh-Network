...
bool SendMessage(float _TEMPERATURE, int _DSTNODE, int _LISTENTIMEOUT = 5000)
{
    ...
    int ERROR_LOG_ = Mesh_Manager.sendtoWait((uint8_t *)MESSAGECHAR_, sizeof(MESSAGECHAR_), _DSTNODE);
    Serial.print("ERROR_LOG_ = ");
    Serial.println(ERROR_LOG_);
    if (ERROR_LOG_ == RH_ROUTER_ERROR_NONE)
    {
        ...
        uint8_t FROM_;
        if (Mesh_Manager.recvfromAckTimeout(BUFFER, &BUFFER_LEN_, _LISTENTIMEOUT, &FROM_))
        {
            // แสดงค่า
            Serial.println();
...
void ListenAndRoute(int _LISTENTIMEOUT)
{
    ...
    if (Mesh_Manager.recvfromAckTimeout(BUFFER, &BUFFER_LEN_, _LISTENTIMEOUT, &FROM_HOP_))
    {
        // แสดงค่า
        Serial.println();
...
void loop()
{
    ...
    // /* สำหรับ Test */
    SendMessage(TEMPERATURE, 250, LISTENTIMEOUT);
...
//     }
//     // set ค่าความถี่ และ spreading factor
//     // RF95_Sender.setModemConfig(RF95_Sender.Bw31_25Cr48Sf512);
//     RF95_Sender.setFrequency(923.2);
//     RF95_Sender.setCADTimeout(500);
//     RF95_Sender.setSignalBandwidth(500000);
//     RF95_Sender.setCodingRate4(5);
//     RF95_Sender.setSpreadingFactor(7);
//     // Mesh_Manager.setTimeout(30000);
//     // RF95_Sender.setSpreadingFactor(7);
//     // RF95_Sender.setSignalBandwidth(250000);
//     RF95_Sender.setTxPower(20, false);
//     Serial.println();
//     Serial.println("Setup RF complete");
//     Serial.println();
...
//     int SEND_START_ = millis(); // จับเวลาในการส่ง
//     int ERROR_LOG_ = Mesh_Manager.sendtoWait((uint8_t *)MESSAGECHAR_, sizeof(MESSAGECHAR_), _DSTNODE);
//     Serial.println("ERROR_LOG" + ERROR_LOG_)    ;
...
//         uint8_t BUFFER_LEN_ = sizeof(BUFFER);
//         uint8_t FROM_;
//         if (Mesh_Manager.recvfromAckTimeout(BUFFER, &BUFFER_LEN_, 10000, &FROM_))
//         {
//             // แสดงค่า
//             Serial.println();
//             Serial.println("Reply From node : " + (String)FROM_);
...
//     //         OLDTIME = CURRENTIME;
//     //     }
//     //     while (!SendMessage(TEMPERATURE, 250))
//     //         ;
//     //     Serial.println("(New)  OLDTIME = " + (String)OLDTIME);
//     // }
//     // else
